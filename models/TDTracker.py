...
class Model(nn.Module):
    def __init__(self, args=None):
        ...
        self.conv1_spatial  = nn.Sequential(nn.Conv3d(2, 32, kernel_size=(1,7,7), stride=(1,1,1), padding=(0,3,3)),
                                            nn.BatchNorm3d(num_features=32),
                                            nn.ReLU()) 
        self.conv1_temporal = nn.Sequential(nn.Conv3d(32, 32, kernel_size=(3,3,3),padding=(1,1,1),bias=False,dilation=(1,1,1)),
                                        nn.BatchNorm3d(num_features=32),
                                        nn.ReLU(),
                                        nn.AvgPool3d((1,3,3)))
...
if __name__ == '__main__':
    ...
    model = Model(args=None).cuda()
    flops = FlopCountAnalysis(model, torch.rand(1, 1,2, 60, 80).float().cuda())
